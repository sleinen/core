...
class RepairOrphanedSubshareTest extends TestCase {
	...
	public function testPopulateDBAndRemoveOrphanShares() {
		...
		for($i=1; $i <= 10; $i++) {
			...
			if ($i <= 5) {
				...
				if ($i === 6) {
					...
					//echo "\nAt 6 the value of parentReshareCount = $parentReshareCount\n";
					/*if ($grabIds[0]['id'] !== 1) {
						$parentReshareCount = $grabIds[0]['id'];
						echo "\nAt 6 the value of parentReshareCount = $parentReshareCount\n";
					}*/
		...
		$expectedCount = 2;
		/*if (in_array(['parent' => null], $results, true) === true) {
			++$expectedCount;
		}*/
			...
			for($i=1; $i <= 100; $i++) {
				...
				if ($userIndex === 0) {
					...
					if (($firstIdSet === false) && ($i === 1)) {
						...
						echo "\nThe id starts from " . $grabIds[0]['id'] . "\n";
						$firstIdSet = true;
					}
